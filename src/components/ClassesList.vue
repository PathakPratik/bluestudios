<template>
  <div :class="{'main-container': !detectMobile}">
      <div v-for="(data,date) in list" :key="date" class="card-top">
        <p ref="dateBlock" :class="{'date-mobile-heading': detectMobile, 'date-heading': !detectMobile}" class="" :data-date="date">{{ formatDate(date) }}</p>
  ...
  methods:{
    selectDate(){
        ...
        for( const n in dateBlocks) {
            const top = window.scrollY || window.scrollTop || document.getElementsByTagName("html")[0].scrollTop;
        ...
        this.$store.dispatch("setSelectedDate", dateBlocks[current].getAttribute('data-date'));
    },
    formatDate(date){
        const stdDate = date.substring(0,4)+'-'+date.substring(4,6)+'-'+date.substring(6,8);
